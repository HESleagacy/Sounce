...
# google-genai retries on its own, but only for retriable API status codes,
# httpx.TimeoutException and httpx.ConnectError. A connection dropped
# mid-request surfaces as httpx.RemoteProtocolError, which that predicate does
# not match, so it reraises on the first attempt -- which is how a transient
# blip was costing whole replies.
...
class GeminiProvider:
    ...
    def _retrying(self, description: str, call: Callable[[], T]) -> T:
        import httpx
    ...
    def localize_response(self, response: str, language: str) -> str:
        ...
        prompt = f"Rewrite this WhatsApp reply entirely in BCP-47 language {language!r}:\n\n{response}"
        localized = self._retrying(
            "Gemini localization",
